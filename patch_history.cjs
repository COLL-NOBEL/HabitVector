              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 rounded-3xl p-6 shadow-sm">
                <div className="flex justify-between items-center pb-6 border-b border-slate-100">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setCurrentWeekOffset(0)} className={\`px-3 py-1.5 rounded-lg text-sm font-semibold \${currentWeekOffset < 1 ? "bg-amber-500 text-slate-900" : "bg-slate-200 dark:bg-slate-800"}\`}>Past Weeks</button>
                ...
                <div className="pt-6">
                  {currentWeekOffset < 1 ? (
                    <div className="space-y-6">
                      {Array.from(new Set([0, ...timetable.map(t => t.weekIndex)])).filter(w => w < 1).sort((a,b) => b - a).map(weekIdx => {
...
const originalHistoryViewRegex = /\{\/\* 3\. TO-DO TAB \*\/\}/;
// Actually we need to replace the existing history block first
const existingHistoryBlockRegex = /\{activeTab === "history" && \([\s\S]*?id="view-history"[\s\S]*?\}\s*\)\}/;

code = code.replace(existingHistoryBlockRegex, historyView.trim());
