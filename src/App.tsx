...
import { TimetableItem, TodoItem, CoachingTip, Opportunity, OpportunitySource, HabitRecommendation, UserGoalProfile, Plan } from "./types";
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<"dashboard" | "timetable" | "todo" | "history" | "opportunities" | "habits" | "settings">("dashboard");
  ...
  const handleGeneratePlan = async (userProfile: UserGoalProfile) => {
    ...
    try {
      const response = await fetch("/api/generate-plan", {
        method: "POST",
        headers: { "Content-Type": "application/json", ...(customApiKey ? { "x-gemini-api-key": customApiKey } : {}) },
        body: JSON.stringify(userProfile),
      });
      const data = await response.json();
      if (response.ok) {
        // Map server results to standard UI format (adding ids where missing)
        const formattedTimetable = (data.timetable || []).map((item: any, idx: number) => ({
          ...item,
          id: `time-${Date.now()}-${idx}`
        }));
        const formattedTodoList = (data.todoList || []).map((item: any, idx: number) => ({
          ...item,
      ...
      console.error(err);
      setApiError("Error connecting to server. Make sure your GEMINI_API_KEY is configured.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleModifyTimetable = async (prompt: string) => {
    try {
      const response = await fetch("/api/modify-timetable", {
        method: "POST",
        headers: { "Content-Type": "application/json", ...(customApiKey ? { "x-gemini-api-key": customApiKey } : {}) },
        body: JSON.stringify({ prompt, currentTimetable: timetable, userProfile: profile }),
      });
      const data = await response.json();
      if (response.ok) {
        const formattedTimetable = (data.timetable || []).map((item: any, idx: number) => ({
        ...
        if (currentPlanId) {
          setPlans(prev => prev.map(p => 
            p.id === currentPlanId ? { ...p, timetable: formattedTimetable } : p
          ));
        }
      } else {
  ...
  const fetchOpportunities = async (interestsTerm: string, customQuery?: string) => {
    setOppsLoading(true);
    try {
      const response = await fetch("/api/discover-opportunities", {
        method: "POST",
        headers: { "Content-Type": "application/json", ...(customApiKey ? { "x-gemini-api-key": customApiKey } : {}) },
        body: JSON.stringify({ interests: interestsTerm, query: customQuery }),
      });
      const data = await response.json();
      if (response.ok) {
  ...
  const handleExportICS = () => {
    ...
    let icsContent = "BEGIN:VCALENDARVERSION:2.0PRODID:-//HabitPulse//NONSGML v1.0//ENCALSCALE:GREGORIANMETHOD:PUBLISH";
    
    timetable.forEach((item) => {
      // Simulate real recurring/timed calendar event
      const cleanTitle = item.activity.replace(/[,;]/g, "");
      const cleanDesc = `${item.description} - Focus Rule: 1% compounding effort. Category: ${item.category}`.replace(/[,;]/g, "");
    ...
    setTodoList(prev => prev.map(item => {
      if (item.id === id) {
        ...
        if (nextStatus === "completed") {
          setStreak(s => s + 1);
          setTimetable(tt => tt.filter(t => !(t.taskId === id && t.weekIndex > currentWeekOffset)));
        } else {
  ...
  const handleAddManualTodo = (e: React.FormEvent) => {
    ...
    const newItem: TodoItem = {
      ...
      estimatedMinutes: 30,
      atomicActionStep: newTodoAtomic || "Open editor for 2 minutes and sit down.",
  ...
  const handleAddManualTime = (e: React.FormEvent) => {
    ...
    const newItem: TimetableItem = {
      ...
      description: newTimeDesc || "Manually scheduled compounding deep work block.",
      colorPreset: "amber",
  ...
  const handleImportOpportunityToTodo = (opp: Opportunity) => {
    ...
    setTodoList([newItem, ...todoList]);
    alert(`"${opp.title}" has been successfully added to your prioritised To-Do List!`);
  };
  ...
  const handleImportHabitToTodo = (rec: HabitRecommendation) => {
    ...
    setTodoList([newItem, ...todoList]);
    alert(`"${rec.title}" habit challenge added directly to your To-Do list!`);
  };


  const handleUpdateTodoDate = (id: string, field: 'startDate' | 'endDate', value: string) => {
    setTodoList(prev => {
      const next = prev.map(item => item.id === id ? { ...item, [field]: value } : item);
      if (currentPlanId) {
        setPlans(plansPrev => plansPrev.map(p => 
  ...
  const handleGenerateTimetableFromTasks = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (response.ok) {
        const newMissions = (data.timetable || []).map((item: any, idx: number) => ({
          ...item,
          id: `time-${Date.now()}-${idx}`,
          status: "pending"
        }));
        ...
        if (currentPlanId) {
          setPlans(plansPrev => plansPrev.map(p => 
            p.id === currentPlanId ? { ...p, timetable: nextTimetable } : p
          ));
        }
        setActiveTab("timetable");
      } else {
        setApiError(data.error || "Failed to generate timetable missions.");
      }
    } catch (err) {
      console.error(err);
  ...
  if (!profile.setupDone) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-4">
        <AIPlannerSetup onGenerate={handleGeneratePlan} loading={loading} />
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col md:flex-row font-sans text-slate-900 dark:text-slate-100 antialiased transition-colors print:bg-white dark:bg-slate-900 print:text-black" id="main-container">
      
      {/* Sidebar Navigation - Styled with Professional Polish Deep Theme */}
      <aside className="w-full md:w-64 bg-white dark:bg-slate-900 flex flex-col shrink-0 border-r border-slate-200 dark:border-slate-800 print:hidden" id="sidebar-nav">
        <div className="p-6 flex items-center justify-between border-b border-slate-200 dark:border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-slate-900 dark:text-white font-extrabold text-base tracking-tight uppercase">HabitVector</h1>
              <span className="text-blue-400 font-mono text-[10px] uppercase tracking-wider block">Custom Plan Builder</span>
            </div>
          </div>
          <button 
        ...
        <div className="p-4 bg-slate-950/60 m-4 rounded-2xl border border-slate-200 dark:border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] text-slate-500 dark:text-slate-400 dark:text-slate-400 uppercase font-bold tracking-wider">Compounding Streak</p>
              <div className="flex items-baseline gap-1">
                <span className="text-2xl font-black text-slate-900 dark:text-white">{streak}</span>
                <span className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">Days 1% Better</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {plans.length > 0 && (
          <div className="px-4 mb-2">
            <div className="flex items-center justify-between mb-1">
              <label className="text-[10px] text-slate-500 dark:text-slate-400 dark:text-slate-400 uppercase font-bold tracking-wider ml-1">Saved Plans</label>
              <button 
                onClick={() => setProfile(p => ({...p, setupDone: false}))}
        ...
        <div className="p-4 m-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800">
          ...
          <h4 className="text-slate-900 dark:text-white text-xs font-bold">Google Gemma Free Credits</h4>
          <p className="text-[10px] text-slate-500 dark:text-slate-400 dark:text-slate-400 mt-1">Get up to $300 cloud credits. Connect your account today.</p>
          <button 
            onClick={() => {
              alert("Feature Demonstration: In production, sponsors like Google pay to feature credits directly here based on user learning topics!");
            }}
      ...
      {/* Main Panel Content */}
      <main className="flex-1 flex flex-col bg-slate-50 dark:bg-slate-900 min-w-0" id="main-content">
        
        {/* Header - Styled with Professional Polish Top bar */}
        <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 flex items-center justify-between px-6 shrink-0 z-10" id="header-bar">
          <div className="flex items-center gap-3">
            <h2 className="text-lg font-bold tracking-tight text-slate-900 dark:text-white">
              {activeTab === "dashboard" && "My Atomic Habits Dashboard"}
              ...
              {activeTab === "opportunities" && "Live Sourced Sponsoring & Tech Opportunities"}
              {apiError && (
                <div className="mt-4 p-3 bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 text-sm font-semibold rounded-xl border border-rose-100 dark:border-rose-900/50 flex justify-between items-center w-full">
                  <div>
                    <p>{apiError}</p>
                    {apiError.includes("429") || apiError.includes("quota") || apiError.includes("Rate limit") ? (
                      <p className="mt-1 text-xs text-rose-500 font-medium">Tip: Add your own Gemini API Key in the Settings tab to avoid rate limits.</p>
                    ) : null}
                  </div>
                  <button onClick={() => setApiError(null)} className="text-rose-400 hover:text-rose-600 self-start mt-1">✕</button>
                </div>
              )}
              {activeTab === "habits" && "Habit-Stacking Booster Recommendations"}
            </h2>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <div onClick={() => setIsProfileModalOpen(true)} className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 flex items-center justify-center cursor-pointer hover:ring-2 ring-amber-500 transition-all" title="Edit Profile">
        ...
        {/* Tab/Content Areas */}
        <div className="flex-1 p-6 overflow-y-auto" id="content-scroller">
          ...
          {timetable.length === 0 && !loading && (
            <div className="mb-6 p-4 bg-amber-50 border border-amber-200 text-amber-800 rounded-2xl flex items-start gap-3">
              ...
              <div>
                <h4 className="font-bold text-sm">System Empty</h4>
                <p className="text-xs">No planner is active. Click "Regenerate Planner" above to brainstorm elements for your exact goals!</p>
              </div>
            </div>
          )}
              ...
              <div className="bg-gradient-to-r from-blue-900 to-slate-900 text-slate-900 dark:text-white rounded-3xl p-6 shadow-md relative overflow-hidden">
                ...
                <div className="relative z-10 max-w-3xl">
                  ...
                  <p className="text-slate-300 dark:text-slate-300 text-sm italic">
                    — Atomic Habits by James Clear. Your blueprint is structured with {profile.hoursPerDay} hours of study per day, leaving you plenty of guilt-free time for gaming and resting.
                  </p>
                </div>
              </div>
              ...
              {coachingTips.length > 0 && (
                <div className="grid md:grid-cols-3 gap-4">
                  {coachingTips.map((tip, idx) => (
                    <div key={idx} className="p-4 bg-white dark:bg-slate-900 border border-slate-200 rounded-2xl shadow-sm flex flex-col justify-between">
                      <div>
                        ...
                        <p className="text-xs text-slate-600 dark:text-slate-400 dark:text-slate-400 mb-4">{tip.explanation}</p>
                      </div>
                      <div className="bg-slate-50 dark:bg-slate-900 p-2.5 rounded-xl border border-slate-100">
                        <span className="text-[10px] text-slate-500 dark:text-slate-400 dark:text-slate-400 font-bold uppercase block">Action Challenge:</span>
                        <p className="text-xs font-semibold text-slate-800 dark:text-slate-200">{tip.actionableChallenge}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="grid lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 p-5 shadow-sm flex flex-col h-[520px]">
                  <div className="flex items-center justify-between pb-4 border-b border-slate-100 shrink-0">
                    <div>
                      <h3 className="font-extrabold text-sm text-slate-900 dark:text-white uppercase tracking-wider">Deep Work Timetable</h3>
                        <p className="text-xs text-slate-500 dark:text-slate-400">Week {currentWeekOffset} | Your planned micro-habits</p>
                      </div>
                      <div className="flex gap-2 print:hidden">
                        <button onClick={() => setCurrentWeekOffset(prev => prev - 1)} className="px-2 py-1 bg-slate-200 dark:bg-slate-700 rounded text-xs">Prev</button>
                        <button onClick={() => setCurrentWeekOffset(0)} className="px-2 py-1 bg-amber-500 text-slate-900 rounded text-xs font-bold">Current</button>
                        <button onClick={() => setCurrentWeekOffset(prev => Math.min(1, prev + 1))} disabled={currentWeekOffset >= 1} className={`px-2 py-1 rounded text-xs ${currentWeekOffset >= 1 ? "bg-slate-100 dark:bg-slate-800 text-slate-400 cursor-not-allowed" : "bg-slate-200 dark:bg-slate-700"}`}>Next</button>
                    </div>
                    <button 
                      onClick={() => setActiveTab("timetable")}
                  ...
                  <div className="flex-1 overflow-y-auto py-4 space-y-4 pr-1">
                    {timetable.slice(0, 5).map((item) => (
                      <div key={item.id} className="flex gap-3">
                        <span className="text-xs font-mono text-slate-500 dark:text-slate-400 dark:text-slate-400 w-12 shrink-0 pt-1">{item.timeRange.split("-")[0]}</span>
                        <div className={`flex-1 p-3 rounded-xl border-l-4 border-y border-r border-slate-150 ${
                          item.colorPreset === "blue" ? "bg-blue-50/50 border-l-blue-500" :
                          item.colorPreset === "emerald" ? "bg-emerald-50/50 border-l-emerald-500" :
                          item.colorPreset === "amber" ? "bg-amber-50/50 border-l-amber-500" :
                          item.colorPreset === "purple" ? "bg-purple-50/50 border-l-purple-500" :
                          item.colorPreset === "rose" ? "bg-rose-50/50 border-l-rose-500" :
                          "bg-indigo-50/50 border-l-indigo-500"
                        }`}>
                          <div className="flex items-center justify-between">
                            <h4 className="text-xs font-extrabold text-slate-900 dark:text-white">{item.activity}</h4>
                          ...
                          <p className="text-[11px] text-slate-600 dark:text-slate-400 dark:text-slate-400 mt-1 line-clamp-1">{item.description}</p>
                        </div>
                      </div>
                    ))}
                    {timetable.length > 5 && (
                      <p className="text-center text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400 italic pt-2">
                        + {timetable.length - 5} more items. Check "Time Table" tab.
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="lg:col-span-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 p-5 shadow-sm flex flex-col h-[520px]">
                  <div className="flex items-center justify-between pb-4 border-b border-slate-100 shrink-0">
                    <div>
                      <h3 className="font-extrabold text-sm text-slate-900 dark:text-white uppercase tracking-wider">My Priorities</h3>
                      <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">Atomic Habits action checklist</p>
                    </div>
                    <button 
                      onClick={() => setActiveTab("todo")}
                      className="text-xs font-semibold text-blue-600 hover:underline"
                  ...
                  <div className="flex-1 overflow-y-auto py-4 space-y-3 pr-1">
                    {todoList.slice(0, 6).map((item) => (
                      <div 
                        ...
                        className={`flex items-start gap-3 p-3 border rounded-xl hover:border-blue-200 transition-colors ${
                          item.status === "completed" ? "bg-slate-50 dark:bg-slate-900/50 opacity-60" : "bg-white dark:bg-slate-900"
                        }`}
                      >
                        <input 
                          type="checkbox" 
                        ...
                        <div className="flex-1 min-w-0">
                          <p className={`text-xs font-bold text-slate-900 dark:text-white ${item.status === "completed" ? "line-through text-slate-500 dark:text-slate-400 dark:text-slate-400" : ""}`}>
                            {item.title}
                          </p>
                          <div className="flex items-center gap-1.5 mt-1">
                            ...
                            <span className="text-[10px] text-slate-500 dark:text-slate-400 dark:text-slate-400">{item.category}</span>
                          </div>
                          {item.atomicActionStep && (
                            <div className="mt-1.5 text-[10px] bg-amber-500/5 text-amber-700 p-1.5 rounded-lg border border-amber-500/10 font-mono">
                              <span className="font-bold">2-Min Trigger:</span> {item.atomicActionStep}
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    {todoList.length > 6 && (
                      <p className="text-center text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400 italic pt-2">
                        + {todoList.length - 6} more pending tasks.
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="lg:col-span-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 p-5 shadow-sm flex flex-col h-[520px]">
                  <div className="flex items-center justify-between pb-4 border-b border-slate-100 shrink-0">
                    <div>
                      <h3 className="font-extrabold text-sm text-slate-900 dark:text-white uppercase tracking-wider">Opportunity Feed</h3>
                      <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">Scholarships, Hackathons, Free Tools</p>
                    </div>
                    <button 
                      onClick={() => setActiveTab("opportunities")}
                      className="text-xs font-semibold text-blue-600 hover:underline"
                  ...
                  <div className="flex-1 overflow-y-auto py-4 space-y-3 pr-1">
                    {oppsLoading ? (
                      <div className="flex flex-col items-center justify-center py-12 text-center">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">Scouting latest opportunities...</p>
                      </div>
                    ) : (
                      opportunities.slice(0, 3).map((opp, idx) => (
                        <div key={idx} className="p-3 bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-2xl relative overflow-hidden">
                          <span className="text-[9px] font-black text-amber-400 uppercase tracking-wide">
                            {opp.badge}
                          </span>
                          <h4 className="text-xs font-extrabold mt-1 text-slate-900 dark:text-white">{opp.title}</h4>
                          <p className="text-[10px] text-slate-300 dark:text-slate-300 mt-1 line-clamp-2">{opp.description}</p>
                          <div className="flex items-center justify-between mt-3 pt-2.5 border-t border-slate-200 dark:border-slate-800">
                            <span className="text-[9px] text-slate-500 dark:text-slate-400 dark:text-slate-400">{opp.provider}</span>
                            <button
                              onClick={() => handleImportOpportunityToTodo(opp)}
              ...
              {currentWeekOffset < 0 ? (
                <div className="bg-white dark:bg-slate-900 border border-slate-200 rounded-3xl p-6 shadow-sm">
                  ...
                  <div className="pt-6 space-y-6">
                    {Array.from<number>(new Set(timetable.map(t => t.weekIndex))).filter(w => w < 0).sort((a,b) => b - a).map(weekIdx => {
            ...
            <div className="space-y-6" id="view-history">
              <div className="bg-white dark:bg-slate-900 border border-slate-200 rounded-3xl p-6 shadow-sm">
                <div className="flex justify-between items-center pb-6 border-b border-slate-100">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setCurrentWeekOffset(-1)} className={`px-3 py-1.5 rounded-lg text-sm font-semibold ${currentWeekOffset < 1 ? "bg-amber-500 text-slate-900" : "bg-slate-200 dark:bg-slate-800"}`}>Prev Week</button>
                ...
                <div className="pt-6">
                  {currentWeekOffset < 1 ? (
                    <div className="space-y-6">
                      {Array.from<number>(new Set([0, ...timetable.map(t => t.weekIndex)])).filter(w => w <= 0).sort((a,b) => b - a).map(weekIdx => {
                        const weekItems = timetable.filter(t => t.weekIndex === weekIdx);
                        return (
                          <div key={weekIdx} className="border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden">
                            <div className="bg-slate-50 dark:bg-slate-800/50 px-4 py-3 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
            ...
            <div className="space-y-6" id="view-todo">
              <div className="bg-white dark:bg-slate-900 border border-slate-200 rounded-3xl p-6 shadow-sm">
                ...
                <div className="pb-6 border-b border-slate-100">
                  <h3 className="font-extrabold text-base text-slate-900 dark:text-white uppercase tracking-wider mb-4">Create New Task</h3>
                  <form onSubmit={handleAddManualTodo} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    <div className="md:col-span-2">
                      <label className="block text-slate-500 dark:text-slate-400 dark:text-slate-400 text-[10px] uppercase font-bold mb-1.5">Task Description / Goal</label>
                      <input
                        type="text"
                        value={newTodoTitle}
                    ...
                    <div>
                      <label className="block text-slate-500 dark:text-slate-400 dark:text-slate-400 text-[10px] uppercase font-bold mb-1.5">Priority</label>
                      <select
                ...
                <div className="py-6">
                  <div className="flex items-center justify-between mb-4">
                    <h4 className="font-extrabold text-sm text-slate-900 dark:text-white uppercase tracking-wider">My Compounding List</h4>
                    <button onClick={handleGenerateTimetableFromTasks} className="text-xs font-bold px-3 py-1.5 bg-indigo-100 text-indigo-700 hover:bg-indigo-200 rounded flex items-center gap-1.5 transition-colors">
                      <Sparkles className="w-3.5 h-3.5" /> Generate Missions from Tasks
                    </button>
                  </div>
                  ...
                  <div className="space-y-3">
                    {todoList.map((item) => (
                      <div 
                        ...
                        className={`flex items-start justify-between p-4 border rounded-2xl transition-all ${
                          item.status === "completed" ? "bg-slate-50 dark:bg-slate-900 border-slate-200 opacity-60" : "bg-white dark:bg-slate-900 border-slate-200/80 hover:border-slate-300 shadow-sm"
                        }`}
                      >
                        <div className="flex items-start gap-4">
                          ...
                          />
                          <div>
                            <p className={`text-sm font-extrabold text-slate-900 dark:text-white ${item.status === "completed" ? "line-through text-slate-500 dark:text-slate-400 dark:text-slate-400" : ""}`}>
                              {item.title}
                            </p>
                            ...
                            <div className="flex items-center gap-2 mt-2">
                              <div className="flex flex-col">
                                ...
                                <input type="date" value={item.startDate || ''} onChange={(e) => handleUpdateTodoDate(item.id, 'startDate', e.target.value)} className="text-xs px-2 py-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 rounded" />
                              </div>
                              <div className="flex flex-col">
                                <label className="text-[9px] font-bold text-slate-500 uppercase">End Date</label>
                                <input type="date" value={item.endDate || ''} onChange={(e) => handleUpdateTodoDate(item.id, 'endDate', e.target.value)} className="text-xs px-2 py-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 rounded" />
                              </div>
                            </div>
                            ...
                            {item.atomicActionStep && (
                              <div className="mt-3 bg-amber-500/5 text-amber-800 p-2.5 rounded-xl border border-amber-500/10 font-mono text-[11px] max-w-xl">
                                <span className="font-bold">⚡ 2-Minute Habit Starter Cue:</span> {item.atomicActionStep}
                              </div>
                            )}
                            ...
                            {item.resources && item.resources.length > 0 && (
                              <div className="mt-2.5 flex flex-wrap items-center gap-2">
                                <span className="text-[10px] text-slate-500 dark:text-slate-400 dark:text-slate-400 font-bold uppercase">FREE SOURCES:</span>
                                {item.resources.map((linkStr, idx) => (
                                  <a
                                    key={idx}
                                    href={linkStr.startsWith("http") ? linkStr : `https://www.google.com/search?q=${encodeURIComponent(linkStr)}`}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="inline-flex items-center gap-1 text-[10px] font-semibold text-blue-600 hover:underline"
                                  >
                                    <span>{linkStr.length > 30 ? "Resource Link" : linkStr}</span>
                    ...
                    {todoList.length === 0 && (
                      <div className="py-12 text-center">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">All habits and targets completed! Add some more above.</p>
                      </div>
                    )}
                  </div>
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 rounded-3xl p-6 shadow-sm">
                ...
                <div className="pb-6 border-b border-slate-100">
                  <div className="max-w-2xl">
                    <h3 className="font-extrabold text-base text-slate-900 dark:text-white uppercase tracking-wider mb-2">Search the Grounded Web</h3>
                    <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400 mb-4">
                      Enter customized domains or categories (e.g. "free nodejs databases", "Web dev hackathons summer 2026", "Outreachy remote internship criteria"). 
                      Gemini will query the live web utilizing Google Search Grounding to find verified active entries.
                    </p>
                    <div className="flex gap-3">
                      ...
                      <button
                        onClick={() => fetchOpportunities(profile.interests, searchQuery)}
                ...
                <div className="py-6">
                  <div className="flex items-center justify-between mb-6">
                    <h4 className="font-extrabold text-sm text-slate-900 dark:text-white uppercase tracking-wider">
                      {searchQuery ? `Verified Search Results for "${searchQuery}"` : "Tailored Dev Opportunities"}
                    </h4>
                    {oppsLoading && <span className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400 animate-pulse">Consulting live database...</span>}
                  </div>

                  <div className="grid md:grid-cols-2 gap-6">
                    {opportunities.map((opp, idx) => (
                      <div key={idx} className="p-5 bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between">
                        <div>
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <span className="text-[10px] text-slate-500 dark:text-slate-400 dark:text-slate-400 font-mono">{opp.type}</span>
                          </div>
                          
                          <h4 className="text-sm font-extrabold text-slate-900 dark:text-white mb-2">{opp.title}</h4>
                          <p className="text-xs text-slate-300 dark:text-slate-300 mb-4">{opp.description}</p>
                          ...
                          <div className="space-y-2 mb-4">
                            <div className="p-2.5 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 text-[11px]">
                              <span className="text-blue-400 font-bold block">Benefit / Support Package:</span>
                              <p className="text-slate-300 dark:text-slate-300 mt-0.5">{opp.benefits}</p>
                            </div>
                            <div className="p-2.5 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 text-[11px]">
                              <span className="text-emerald-400 font-bold block">Requirements:</span>
                              <p className="text-slate-300 dark:text-slate-300 mt-0.5">{opp.requirements}</p>
                            </div>
                          </div>
                        </div>

                        <div className="pt-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
                          <span className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400 font-bold">Provider: {opp.provider}</span>
                          <div className="flex gap-2">
                            <a
                              href={opp.actionLink.startsWith("http") ? opp.actionLink : `https://www.google.com/search?q=${encodeURIComponent(opp.actionLink)}`}
                              target="_blank"
                              rel="noopener noreferrer"
                  ...
                  {sources.length > 0 && (
                    <div className="mt-8 p-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 rounded-2xl">
                      <h5 className="text-xs font-extrabold text-slate-900 dark:text-white uppercase tracking-wide mb-2">Google Grounding References</h5>
                      <div className="flex flex-wrap gap-3">
                        {sources.map((source, idx) => (
                          ...
                          >
                            <span>{source.title || "Search Reference"}</span>
                            <ExternalLink className="w-3 h-3" />
                          </a>
                        ))}
                      </div>
          ...
          {activeTab === "habits" && (
            <div className="space-y-6" id="view-habits">
              <div className="bg-white dark:bg-slate-900 border border-slate-200 rounded-3xl p-6 shadow-sm">
                <div className="pb-6 border-b border-slate-100">
                  <h3 className="font-extrabold text-base text-slate-900 dark:text-white uppercase tracking-wider mb-1">Habit-Stacking Core Recommendations</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">
                    Proven incremental growth workflows modeled directly on compounding career progress. Click to import them straight to your task schedule.
                  </p>
                </div>
                ...
                <div className="py-6">
                  {habitsLoading ? (
                    <div className="flex flex-col items-center justify-center py-12 text-center">
                      <div className="w-8 h-8 border-2 border-blue-500 border-t-transparent rounded-full animate-spin mb-3"></div>
                      <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">Loading daily compounding habit options...</p>
                    </div>
                  ) : (
                    <div className="grid md:grid-cols-2 gap-6">
                      {habitRecommendations.map((rec, idx) => (
                        <div key={idx} className="p-5 bg-white dark:bg-slate-900 border border-slate-200 rounded-3xl hover:border-blue-300 transition-colors shadow-sm flex flex-col justify-between">
                          <div>
                            <div className="flex items-center justify-between mb-3">
                              <span className="bg-blue-50 text-blue-700 text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-wider">
                                {rec.category}
                              </span>
                              <span className="text-[10px] text-slate-500 dark:text-slate-400 dark:text-slate-400 font-mono">Est: {rec.estimatedMinutes} mins</span>
                            </div>

                            <h4 className="text-sm font-extrabold text-slate-900 dark:text-white mb-1">{rec.title}</h4>
                            <span className="text-[10px] text-amber-600 font-bold block mb-3">Principle: {rec.habitPrinciple}</span>
                            
                            <p className="text-xs text-slate-600 dark:text-slate-400 dark:text-slate-400 mb-4">{rec.description}</p>

                            <div className="p-3 bg-slate-50 dark:bg-slate-900 border border-slate-150 rounded-2xl text-xs mb-4">
                              <span className="font-bold text-slate-900 dark:text-white block mb-1">Step-By-Step / Trigger Workflow:</span>
                              <p className="text-slate-600 dark:text-slate-400 dark:text-slate-400 italic">{rec.actionSteps}</p>
                            </div>
                          </div>

                          <div className="pt-4 border-t border-slate-100 flex justify-end">
                            <button
                              onClick={() => handleImportHabitToTodo(rec)}
            ...
            <div className="space-y-6" id="view-settings">
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm">
                <div className="pb-6 border-b border-slate-100 dark:border-slate-800">
                  <h3 className="font-extrabold text-base text-slate-900 dark:text-white uppercase tracking-wider mb-1">App Settings</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Configure your preferences, API keys, and custom settings.
                  </p>
                </div>
                ...
                <div className="py-6">
                  <div className="max-w-xl space-y-4">
                    <div>
                      <label className="block text-slate-900 dark:text-white text-sm font-bold mb-2">Gemini API Key</label>
                      <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">
                        Provide your own Gemini API key to avoid rate limits and use your own quota. If left empty, the app will use its default rate-limited key.
                      </p>
                      <input
                        type="password"
                        value={customApiKey}
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-2xl shadow-2xl p-6 max-h-[80vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold dark:text-white">Task Plan (All Weeks)</h3>
              <button onClick={() => setSelectedTaskId(null)} className="text-slate-500 hover:text-slate-900 dark:hover:text-white">✕</button>
            </div>
            <div className="space-y-4">
              {timetable.filter(t => t.taskId === selectedTaskId).sort((a,b) => a.weekIndex - b.weekIndex).map(t => (
                <div key={t.id} className="p-3 border border-slate-200 dark:border-slate-800 rounded-lg flex justify-between items-center">
                  <div>
                    <span className="text-xs font-bold bg-amber-500 px-2 py-1 rounded text-slate-900">Week {t.weekIndex}</span>
                    <span className="text-xs font-bold bg-blue-500 px-2 py-1 rounded text-white ml-2">{t.day} {t.timeRange}</span>
                    <p className="text-sm font-semibold dark:text-white mt-1">{t.activity}</p>
                  </div>
                  <span className="text-xs font-bold uppercase text-slate-500 dark:text-slate-400">{t.status}</span>
                </div>
              ))}
              {timetable.filter(t => t.taskId === selectedTaskId).length === 0 && (
                <p className="text-slate-500 dark:text-slate-400 text-sm">No missions assigned to this task.</p>
              )}
            </div>
          </div>
        </div>
      ...
      {isProfileModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="w-full max-w-4xl">
            <AIPlannerSetup 
              onGenerate={async (p) => { await handleGeneratePlan(p); setIsProfileModalOpen(false); }} 
              loading={loading} 
