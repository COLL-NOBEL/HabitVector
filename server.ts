...
app.post("/api/generate-plan", async (req, res) => {
  try {
    const { interests, educationLevel, hoursPerDay, daysPerWeek, examDate, currentProblems } = req.body;
    const ai = getAI(req);
        ...
        responseSchema: {
          ...
          properties: {
            timetable: {
              ...
              items: {
                type: Type.OBJECT,
                required: ["id", "weekIndex", "day", "timeRange", "activity", "category", "description", "colorPreset"],
                properties: {
                  id: { type: Type.STRING },
                  taskId: { type: Type.STRING },
                  weekIndex: { type: Type.INTEGER, description: "0 for week 1, 1 for week 2, 2 for week 3, 3 for week 4" },
            ...
            todoList: {
              ...
              items: {
                type: Type.OBJECT,
                required: ["id", "title", "category", "priority", "estimatedMinutes", "atomicActionStep", "period"],
                properties: {
                  id: { type: Type.STRING },
            ...
            coachingTips: {
              ...
              items: {
                type: Type.OBJECT,
                required: ["rule", "explanation", "actionableChallenge"],
                properties: {
...
app.post("/api/generate-timetable-from-tasks", async (req, res) => {
  try {
            ...
            timetable: {
              ...
              items: {
                ...
                properties: {
                  id: { type: Type.STRING },
                  taskId: { type: Type.STRING },
                  weekIndex: { type: Type.INTEGER, description: "0 for week 1, 1 for week 2, etc." },
                  day: { type: Type.STRING, description: "'Monday', 'Tuesday', etc." },
                  timeRange: { type: Type.STRING },
                  activity: { type: Type.STRING },
                  category: { type: Type.STRING },
    ...
    handleApiError(res, error, "Failed to generate timetable missions.");
  }
});

// 2. API Endpoint: Discover tech opportunities & free resources using Google Search Grounding!
app.post("/api/discover-opportunities", async (req, res) => {
  try {
    ...
    const searchTerm = query || `${interests || "web development computer engineering"} free services hackathons competitions open source programs`;
...
app.post("/api/propose-habits", async (req, res) => {
  try {
        ...
        responseSchema: {
          ...
          properties: {
            recommendations: {
              ...
              items: {
                type: Type.OBJECT,
                required: ["title", "category", "habitPrinciple", "description", "actionSteps", "estimatedMinutes"],
                properties: {
                  title: { type: Type.STRING, description: "e.g., 'Share a micro-lesson on LinkedIn'" },
                  category: { type: Type.STRING, description: "e.g., 'Networking', 'Open Source', 'Project', 'Reading'" },
                  habitPrinciple: { type: Type.STRING, description: "The Atomic Habit principle it targets (e.g. 'Show Up', 'Design Environment', 'Habit Stacking')" },
                  description: { type: Type.STRING, description: "Brief explanation of why this creates compounding career benefits" },
                  actionSteps: { type: Type.STRING, description: "Step-by-step guidance on how to do it correctly and legally" },
                  estimatedMinutes: { type: Type.INTEGER, description: "Typical duration" }
                }
              }
            }
          }
...
app.post("/api/modify-timetable", async (req, res) => {
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
          ...
          items: {
            type: Type.OBJECT,
            required: ["day", "timeRange", "activity", "category", "description", "colorPreset"],
            properties: {
